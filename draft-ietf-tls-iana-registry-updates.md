...
- Add the following note:
    ...
    Note(1): Experts are to verify that there is in fact a publicly
    available standard.
...
focus on IANA registries are aware that TLS1.3 {{I-D.ietf-tls-tls13}}
...
for populating the Recommended column:
    ...
    Standards Track RFCs.  Cipher suites marked as "No" are not; cipher
    suites marked "No" range from "good" to "bad" from a cryptographic
    standpoint.

    Note(3): The designated expert {{RFC8126}} only ensures that the
...
IANA [SHALL add/has added] the following note:
    ...
    Note(2): Supported Groups marked as "Yes" are those allocated via
...
- The following note to the TLS Exporter Label Registry:
    ...
    Note(2): {{!RFC5705}} defines keying material exporters for TLS in
    terms of the TLS PRF.  {{I-D.ietf-tls-tls13}} replaced the PRF with
    HKDF, thus requiring a new construction. The exporter interface
    remains the same, however the value is computed different.
...
[SHALL add/has added] the following note:
     ...
     Note(3): The designated expert {{RFC8126}} ensures that the
     specification is publicly available.  The expert also verifies that
     the label is a string consisting of printable ASCII characters
     beginning with "EXPORTER". IANA MUST also verify that one label is
     not a prefix of any other label.  For example, labels "key" or
     "master secretary" are forbidden.

     Note(4): Exporters Labels marked as "Yes" are those allocated via
...
Values registry:
    ...
    Note(2): The following extensions are only applicable to (D)TLS
